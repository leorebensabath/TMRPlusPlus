...
def compute_sim_matrix(model, dataset, keyids, batch_size=256):
    ...
    with torch.inference_mode():
        ...
        text_cat_idx = [action_latent_text_idx[tuple(latent_texts[i].to("cpu").numpy())] for i in range(len(latent_texts))]

        latent_motions = torch.cat(latent_motions)
        motion_cat_idx = []
        for start_ind, end_ind in text_indices:
            motion_cat_idx.append(text_cat_idx[start_ind:end_ind])
...
@hydra.main(version_base=None, config_path="configs", config_name="retrieval_action_multi_labels")
def retrieval_action_multi_labels(newcfg: DictConfig) -> None:
    ...
    from src.model.metrics import all_contrastive_metrics_action_retrieval_multi_labels, print_latex_metrics
    ...
    metrics = all_contrastive_metrics_action_retrieval_multi_labels(sim_matrix, motion_cat_idx, norm_metrics=True)
